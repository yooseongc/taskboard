...
### 일반 원칙 — 테이블은 페이지 레벨 스크롤

표는 **자체 `overflow-x-auto` 래퍼를 두지 않는다.** 대신 `<table>` 에 `min-w-[720px]` 를 주고 `<main>` 의 `overflow-auto` 가 페이지 전체를 좌우로 스크롤하게 한다.

이 패턴이 선호되는 이유:
- 테이블 고유 스크롤바와 페이지 스크롤바가 이중으로 나타나지 않음.
- toolbar / footer / 상단 탭이 테이블과 분리되어 독립 스크롤되는 어색함이 사라짐.
- 모바일에서 touch 제스처가 자연스러움 (테이블 경계 안에서만 스와이프할 필요 없음).

구현:
- `<main className="flex-1 overflow-auto min-w-0">` (Layout.tsx)
- `<div className="flex-1 flex flex-col overflow-hidden min-w-0">` — 메인 영역 래퍼에 `min-w-0` 이 있어야 flex 자식이 content 너비에 끌려가지 않음.
- 각 `<table>` 에 `w-full min-w-[720px] text-sm` — 기본값은 container width, content 가 넓으면 720px 이상 자연 확장.
- `<tfoot className="sticky bottom-0">` 은 세로 스크롤에만 고정 (가로 스크롤 시 footer 도 같이 이동).
- `ManagementPage`, `AdminUsersPage`, `DirectoryPage`, `OrgPage`, `TableView` 가 이 패턴 공유.

### 칸반 (Board view)

- `<div className="flex-1 overflow-x-auto p-4">` 로 kanban 영역은 자체 스크롤. 컬럼이 많을 때 `<main>` 까지 끌고가면 toolbar 도 같이 스크롤되어 어색해짐.
- 각 컬럼: `w-64 md:w-72 flex-shrink-0` — 좁은 화면에서도 읽기 편한 최소 폭 유지.
...
`index.css` 의 `::-webkit-scrollbar` 가 10px (세로·가로 동일). 썸은 `color-mix(in srgb, var(--color-text-muted) 45%, transparent)` — 라이트/다크 모두에서 은은하지만 "스크롤할 수 있음" 이 확실히 보임. Firefox 는 `scrollbar-width: thin` + `scrollbar-color` 동일 톤.
...
- `frontend/src/components/TableView.tsx` — `<table className="w-full min-w-[720px]">` + sticky `<tfoot>` + 집계 푸터
- `frontend/src/components/AccentColorSync.tsx` — 사이드바 색상 preference 적용 시 WCAG 명도 기반 text 토큰 자동 오버라이드
