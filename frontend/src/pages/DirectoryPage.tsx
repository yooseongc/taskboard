    ...
    <>
      <table className="w-full text-sm">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-4 py-2.5 text-left font-medium text-gray-600">Person</th>
            <th className="px-4 py-2.5 text-left font-medium text-gray-600">Email</th>
            <th className="px-4 py-2.5 text-left font-medium text-gray-600">Role in Dept</th>
            <th className="px-4 py-2.5 text-left font-medium text-gray-600">Other Departments</th>
        ...
        <tbody className="divide-y divide-gray-100">
          {paged.map((m) => (
            <tr key={m.user_id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-2.5 text-gray-500">{m.user_email}</td>
              <td className="px-4 py-2.5">
              ...
              <td className="px-4 py-2.5">
                {m.user?.department_ids
                  ...
                  .map((did) => (
                    <span key={did} className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded mr-1">
    ...
    <table className="w-full text-sm">
      <thead className="bg-gray-50">
        <tr>
          <th className="px-4 py-2.5 text-left font-medium text-gray-600">Person</th>
          <th className="px-4 py-2.5 text-left font-medium text-gray-600">Email</th>
          <th className="px-4 py-2.5 text-left font-medium text-gray-600">Roles</th>
          <th className="px-4 py-2.5 text-left font-medium text-gray-600">Departments</th>
          <th className="px-4 py-2.5 text-left font-medium text-gray-600">Status</th>
        </tr>
      </thead>
      <tbody className="divide-y divide-gray-100">
              ...
              <div className="flex flex-wrap gap-1">
                {user.department_ids.length > 0
                  ? user.department_ids.map((did) => (
                      <span key={did} className="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">
                        {deptMap.get(did) ?? did.slice(0, 8)}
                      </span>
                    ))
                  : <span className="text-xs text-gray-300">-</span>}
    ...
    <div className="px-4 py-3 border-t flex items-center justify-between text-sm">
      ...
      <div className="flex gap-1">
        <button
          ...
          className="px-2.5 py-1 border rounded text-xs disabled:opacity-30 hover:bg-gray-50"
        >
          Prev
        </button>
        <button
      ...
      >
        {hasChildren ? (
          <button
            ...
            className="w-3.5 h-3.5 flex items-center justify-center text-gray-400 flex-shrink-0"
          >
            <svg
